...
-(void)initHeaderView //相册的排版刷新
{
    ...
    if (imageCount < MaxImageCount) {
        CGFloat addPicX =padding + (imageCount%4)*(pictureHW+padding);
        CGFloat addPicY =CGRectGetMaxY(reportStateTextView.frame) + padding +(imageCount/4)*(pictureHW+padding);
        CGFloat addPicW =pictureHW;
        CGFloat addPicH =pictureHW;
...
//        UIButton *addPictureButton = [[UIButton alloc]initWithFrame:CGRectMake(padding + (imageCount%4)*(pictureHW+padding), CGRectGetMaxY(reportStateTextView.frame) + padding +(imageCount/4)*(pictureHW+padding), pictureHW, pictureHW)];
        UIButton *addPictureButton =[[UIButton alloc] initWithFrame:CGRectMake(addPicX, addPicY, addPicW, addPicH)];
